...
import { BigInt, BigDecimal, store, Address, BigDecimal } from '@graphprotocol/graph-ts'
import {
  Pair,
  Token,
  UniswapFactory,
...
export function handleSync(event: Sync): void {
  ...
  if (pair !== null && bundle !== null && uniswap !== null) {
...
export function handleMint(event: Mint): void {
  ...
  if (transaction !== null && pair !== null && uniswap !== null && bundle !== null) {
    let mints = transaction.mints
    let mint = MintEvent.load(mints[mints.length - 1])
    let token0 = Token.load(pair.token0)
    let token1 = Token.load(pair.token1)

    if (mint !== null && token0 !== null && token1 !== null) {
      ...
      let a = token0.derivedETH === null ? BigDecimal.fromString("0") : token0.derivedETH

      let amountTotalUSD = token1.derivedETH === null ? BigDecimal.fromString("0") : token1.derivedETH
        .times(token1Amount)
        .plus(a.times(token0Amount))
...
export function handleBurn(event: Burn): void {
  ...
  if (token0 === null || token1 === null || burn === null) {
    return
  }
  let token0Amount = convertTokenToDecimal(event.params.amount0, token0.decimals)
  let token1Amount = convertTokenToDecimal(event.params.amount1, token1.decimals)
...
export function handleSwap(event: Swap): void {
  ...
  let a = token0.derivedETH === null ? BigDecimal.fromString("0") : token0.derivedETH
  let derivedAmountETH = token1.derivedETH === null ? BigDecimal.fromString("0") : token1.derivedETH
    .times(amount1Total)
    .plus(a.times(amount0Total))
